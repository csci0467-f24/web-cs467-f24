...
1. Accept the assignment on our [Github Classroom](https://classroom.github.com/a/Bp_2jqpp).
...
## _A note about speed_

_If you have an older or less capable machine, it may struggle with this piece. The struggle primarily comes from the blur operation we introduce about halfway through._

_Using a smaller canvas may be enough to make it run at a reasonable sped, so you should try that first._

_If reducing the canvas size isn't enough, you can do an alternate version that lowers the transparency on the thick lines and doesn't do the blur step. However, I would like to test it in the original form, so if you need to do this, add a variable at the top of the script called `SLOW_MODE` and set it to `true`. When `SLOW_MODE` is true, your code should make the adjustment above. When it is set to `false`, it should follow the process described below (this will allow me to test your code as intended)._
...
- submit the repository on [Gradescope](https://www.gradescope.com/courses/873354/assignments/5116608) ([submission directions](../resources/gradescope))
